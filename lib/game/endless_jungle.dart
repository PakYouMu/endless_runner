...
class EndlessJungle extends FlameGame with TapDetector, HasCollisionDetection {
  EndlessJungle({super.camera});
  ...
  void onTapDown(TapDownInfo info) {
    // Make _player jump only when game is playing.
  ...
  Future<PlayerData> _readPlayerData() async {
    final playerDataBox =
        await Hive.openBox<PlayerData>('EndlessJungle.PlayerDataBox');
    final playerData = playerDataBox.get('EndlessJungle.PlayerData');

    // If data is null, this is probably a fresh launch of the game.
    if (playerData == null) {
      // In such cases store default values in hive.
      await playerDataBox.put('EndlessJungle.PlayerData', PlayerData());
    }

    // Now it is safe to return the stored value.
    return playerDataBox.get('EndlessJungle.PlayerData')!;
  }

  /// This method reads [Settings] from the hive box.
  Future<Settings> _readSettings() async {
    final settingsBox =
        await Hive.openBox<Settings>('EndlessJungle.SettingsBox');
    final settings = settingsBox.get('EndlessJungle.Settings');

    // If data is null, this is probably a fresh launch of the game.
    if (settings == null) {
      // In such cases store default values in hive.
      await settingsBox.put(
        'EndlessJungle.Settings',
        Settings(bgm: true, sfx: true),
      );
    }

    // Now it is safe to return the stored value.
    return settingsBox.get('EndlessJungle.Settings')!;
  }
